...
1. **Clocks**: 
   ...
   time and date.
   - `std::chrono::steady_clock`: Represents a monotonic clock that cannot be adjusted. It's used for measuring elapsed time and is suitable
...
1. **Performance Metrics**:
   ...
   - **Memory Usage**: Measures how much memory is used by the code, which can be important for understanding the impact of data structures 
   and algorithms.
...
4. **Micro-Benchmarks**:
   - These are benchmarks focused on small pieces of code or individual functions to understand their performance characteristics. They are useful 
...
Benchmarking in C++ involves measuring and analyzing the performance of code to assess execution time, memory usage, and CPU utilization. 
It can be done manually using timing functions from the standard library, with profiling tools for detailed insights, or using specialized 
benchmarking libraries like Google Benchmark. Proper benchmarking practices involve isolating the code, running multiple iterations, and 
analyzing results to make informed optimization decisions.
...
you code slow and give you abnormal benchmarks
also you should know that debug mode is different from release mode so both will take different time to excuete an code 
...
here you can see that the release mode takes 0 sec wtf how is that possible so what does the compiler does it optimize the code in release 
mode where for debug mode that is not the case
While it might depend on the context, the general consensus is to use release mode for benchmarking performance. 
This approach ensures you get accurate and relevant results that reflect how your code will behave in a real-world environment.
but for some cases you should use debug mode 
but when you are benchmarking the whole code you should use release mode 
but when you are finding benchmarking of an specific code which contains can be known at compile time it is more obvious to use debug mode
also note that the debug mode has some overhead so that could also make the benchmarking wrong


now let us see this code in assemblly code of this code in debug mode
...
now let us see this code in assemblly code of this code in release mode
...
you can see that there is no add instruction here like in the debug mode 
the only thing compiler does here is that it push some no on the stack
00E6114C  push        30D40h  
now lets this hexadecimal no to decimal
which is 200000 
...
//seing data in visual form makes it look good and is more readable if you want to do an visual benchmarking then click here
#include"visual_benchmarking.cpp"
